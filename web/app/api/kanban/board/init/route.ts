...
export async function POST(request: Request) {
  ...
  try {
    await fetch(`${url}/rest/v1/rpc/seed_kanban_stages_canonical`, { method:'POST', headers, body: JSON.stringify({ p_org: ctx.org_id }) })
  } catch {}
  
  // Garante que as colunas obrigatórias #1 e #99 sempre existam
  const existing = await fetch(`${url}/rest/v1/kanban_stages?org_id=eq.${ctx.org_id}&select=id,position,name&order=position.asc`, { headers, cache:'no-store' })
  ...
  try {
    const listResp = await fetch(`${url}/rest/v1/kanban_stages?org_id=eq.${ctx.org_id}&select=id,name,position,is_fixed,stage_code&order=position.asc`, { headers, cache:'no-store' })
    ...
    if (Array.isArray(stages) && stages.length > 0) {
      ...
      if (!pos1) {
        ...
        if (pos1 && Number(pos1.position) !== 1) {
          await fetch(`${url}/rest/v1/kanban_stages?id=eq.${pos1.id}&org_id=eq.${ctx.org_id}`, { method:'PATCH', headers, body: JSON.stringify({ position: 1 }) })
        }
      }
      ...
      if (!pos99) {
        ...
        if (pos99 && Number(pos99.position) !== 99) {
          await fetch(`${url}/rest/v1/kanban_stages?id=eq.${pos99.id}&org_id=eq.${ctx.org_id}`, { method:'PATCH', headers, body: JSON.stringify({ position: 99 }) })
        }
      }
      // Reindexar intermediárias (se existirem)
      const refreshed = await (await fetch(`${url}/rest/v1/kanban_stages?org_id=eq.${ctx.org_id}&select=id,position&order=position.asc`, { headers, cache:'no-store' })).json().catch(()=>[])
      ...
      if (mids.length > 0) {
        ...
        for (const s of mids) {
          if (Number(s.position) !== order) {
            await fetch(`${url}/rest/v1/kanban_stages?id=eq.${s.id}&org_id=eq.${ctx.org_id}`, { method:'PATCH', headers, body: JSON.stringify({ position: order }) })
          }
  ...
  try {
    ...
    let sResp = await fetch(`${url}/rest/v1/kanban_stages?org_id=eq.${ctx.org_id}&position=eq.1&select=id,position&limit=1`, { headers, cache:'no-store' })
    let stage = (await sResp.json().catch(()=>[]))?.[0]
    if (!stage?.id) {
      sResp = await fetch(`${url}/rest/v1/kanban_stages?org_id=eq.${ctx.org_id}&select=id,position&order=position.asc&limit=1`, { headers, cache:'no-store' })
      stage = (await sResp.json().catch(()=>[]))?.[0]
    }
    if (stage?.id) {
      // alunos enviados (ou marcados para enviar) sem card
      const studentsResp = await fetch(`${url}/rest/v1/students?org_id=eq.${ctx.org_id}&or=(onboard_opt.eq.enviar,onboard_opt.eq.enviado)&select=id`, { headers, cache:'no-store' })
      const students = await studentsResp.json().catch(()=>[])
      for (const st of students) {
        const exist = await fetch(`${url}/rest/v1/kanban_items?org_id=eq.${ctx.org_id}&student_id=eq.${st.id}&select=id&limit=1`, { headers, cache:'no-store' })
        const has = (await exist.json().catch(()=>[]))?.[0]
        if (!has) {
          const posResp = await fetch(`${url}/rest/v1/kanban_items?org_id=eq.${ctx.org_id}&stage_id=eq.${stage.id}&select=position&order=position.desc&limit=1`, { headers, cache:'no-store' })
          const top = (await posResp.json().catch(()=>[]))?.[0]
          const nextPos = Number.isFinite(Number(top?.position)) ? Number(top.position) + 1 : 0
          await fetch(`${url}/rest/v1/kanban_items`, { method:'POST', headers, body: JSON.stringify({ org_id: ctx.org_id, student_id: st.id, stage_id: stage.id, position: nextPos }) })
        }
      }
    }
  } catch {}
