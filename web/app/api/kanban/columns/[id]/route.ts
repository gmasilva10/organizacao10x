...
export async function PATCH(request: Request, ctxParam: { params: Promise<{ id: string }> }) {
  ...
  const colResp = await fetch(`${url}/rest/v1/onboarding_columns?id=eq.${id}&org_id=eq.${ctx.org_id}&select=title&limit=1`, { headers:{ apikey:key!, Authorization:`Bearer ${key}`! } })
  ...
  const upd = await fetch(`${url}/rest/v1/onboarding_columns?id=eq.${id}&org_id=eq.${ctx.org_id}`, {
...
export async function DELETE(request: Request, ctxParam: { params: Promise<{ id: string }> }) {
  ...
  const cardsResp = await fetch(`${url}/rest/v1/onboarding_cards?org_id=eq.${ctx.org_id}&column_id=eq.${id}&select=id&limit=1`, {
  ...
  const del = await fetch(`${url}/rest/v1/onboarding_columns?id=eq.${id}&org_id=eq.${ctx.org_id}`, {
