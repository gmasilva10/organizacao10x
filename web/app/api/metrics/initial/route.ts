...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [studentsResp, servicesResp, collaboratorsResp, occurrencesResp, kanbanResp] = await Promise.all([
      // Students count
      fetch(`${url}/rest/v1/students?org_id=eq.${ctx.org_id}&deleted_at=is.null&select=id`, {
      ...
      // Collaborators count
      fetch(`${url}/rest/v1/collaborators?org_id=eq.${ctx.org_id}&select=id`, {
        headers: { ...headers, Prefer: "count=exact" },
        cache: "no-store"
      }),
      // Open occurrences count
      fetch(`${url}/rest/v1/student_occurrences?org_id=eq.${ctx.org_id}&status=eq.OPEN&select=id`, {
        headers: { ...headers, Prefer: "count=exact" },
        cache: "no-store"
      }),
      // Kanban items count
      fetch(`${url}/rest/v1/kanban_items?org_id=eq.${ctx.org_id}&select=id`, {
        headers: { ...headers, Prefer: "count=exact" },
        cache: "no-store"
      })
    ])
    ...
    const todayOccurrencesResp = await fetch(
      `${url}/rest/v1/student_occurrences?org_id=eq.${ctx.org_id}&occurred_at=gte.${today}&select=id`,
