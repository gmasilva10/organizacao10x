...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Erro ao buscar vers̵es" }, { status: 500 })
    }
    ...
    console.error('Erro na API de vers̵es:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('Erro ao criar verṣo:', error)
      return NextResponse.json({ error: "Erro ao criar verṣo" }, { status: 500 })
    }
