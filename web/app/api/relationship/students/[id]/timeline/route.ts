...
export async function GET(request: Request, ctxParam: { params: Promise<{ id: string }> }) {
  ...
  const resp = await fetch(`${url}/rest/v1/relationship_messages?org_id=eq.${ctx.org_id}&student_id=eq.${id}&order=created_at.desc`, { headers: { apikey: key!, Authorization: `Bearer ${key}`!, Range: `${rangeStart}-${rangeEnd}`, Prefer: 'count=exact' } })
