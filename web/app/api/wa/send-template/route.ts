...
export async function POST(request: NextRequest) {
  const correlationId = `${new Date().toISOString().replace(/[-:]/g, '').split('.')[0]}-wa-template`
  
  try {
    const body = await request.json()
    const { phone, template, variables, instance, token } = body

    // Validação de parâmetros obrigatórios
    if (!phone || !template || !instance || !token) {
      logAction('SEND_TEMPLATE_ERROR', { phone, template, instance: !!instance, token: !!token }, 400)
      return NextResponse.json(
        { error: 'Parâmetros obrigatórios: phone, template, instance, token' },
        { status: 400 }
      )
    }
    ...
    const zapiUrl = `https://api.z-api.io/instances/${instance}/token/${token}/send-template`
    ...
    logAction('SEND_TEMPLATE_RESPONSE', {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { 
          error: 'Erro ao enviar template no WhatsApp', 
    ...
    logAction('SEND_TEMPLATE_EXCEPTION', { error: error instanceof Error ? error.message : 'Unknown error' }, 500)
    return NextResponse.json(
      { 
