...
export async function PATCH(request: Request, ctxParam: { params: Promise<{ id: string }> }) {
  ...
  const resp = await fetch(`${url}/rest/v1/tenant_users?org_id=eq.${ctx.org_id}&user_id=eq.${id}`, { method:'PATCH', headers:{ apikey:key!, Authorization:`Bearer ${key}`!, 'Content-Type':'application/json', Prefer:'return=minimal' }, body: JSON.stringify(patch) })
