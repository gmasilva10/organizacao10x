...
export async function PATCH(request: Request, ctxParam: { params: Promise<{ id: string }> }) {
  ...
  // audit simplificado
  await fetch(`${url}/rest/v1/settings_audit`, { method:'POST', headers:{ apikey:key!, Authorization:`Bearer ${key}`!, 'Content-Type':'application/json', Prefer:'return=minimal' }, body: JSON.stringify({ org_id: ctx.tenantId, actor_user_id: ctx.userId, area: 'roles', action: 'update', before: null, after: rows }) })
