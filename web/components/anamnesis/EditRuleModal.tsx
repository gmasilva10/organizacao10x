...
interface EditRuleModalProps {
  ...
  rule: {
    id: string
    condition: { all: Array<{ tag: string; op: 'eq' | 'in' | 'gt' | 'lt' | 'gte' | 'lte'; val: string | number | string[] }> }
    outputs: {
      aerobio: {
...
export function EditRuleModal({ isOpen, onClose, onSuccess, rule, versionId }: EditRuleModalProps) {
  const [priority, setPriority] = useState<'critica' | 'alta' | 'media' | 'baixa'>(rule.priority_clinical)
  const [conditions, setConditions] = useState<Condition[]>(rule.condition.all || [{ tag: '', op: 'eq', val: '' }])
  const [outputs, setOutputs] = useState<Outputs>({
    aerobio: { 
      duracao_min: rule.outputs.aerobio.duracao_min, 
      intensidade: rule.outputs.aerobio.intensidade, 
      frequencia_sem: rule.outputs.aerobio.frequencia_sem, 
      obs: rule.outputs.aerobio.obs || [] 
    },
    pesos: { 
      exercicios: rule.outputs.pesos.exercicios, 
      series: rule.outputs.pesos.series, 
      reps: rule.outputs.pesos.reps, 
      frequencia_sem: rule.outputs.pesos.frequencia_sem, 
      intensidade_pct_1rm: rule.outputs.pesos.intensidade_pct_1rm, 
      obs: rule.outputs.pesos.obs || [] 
    },
    flex_mob: { 
      foco: rule.outputs.flex_mob.foco, 
      obs: rule.outputs.flex_mob.obs || [] 
    },
    contraindicacoes: rule.outputs.contraindicacoes || [],
    observacoes: rule.outputs.observacoes || []
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update rule
      const response = await fetch(`/api/guidelines/versions/${versionId}/rules/${rule.id}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          ...
          outputs,
          priority_clinical: priority
        })
      })

      if (!response.ok) {
        const error = await response.json()
        throw new Error(error.message || 'Erro ao atualizar regra')
      }
      ...
      toast.error(error instanceof Error ? error.message : 'Erro ao atualizar regra')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-8 overflow-y-auto max-h-[calc(95vh-180px)] px-6">
          ...
          <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
            <div className="space-y-4">
              ...
              <Button type="button" variant="outline" onClick={handleAddCondition} className="w-full h-12 border-dashed border-2 border-blue-300 text-blue-600 hover:bg-blue-50">
